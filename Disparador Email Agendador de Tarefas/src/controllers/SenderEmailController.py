...
from src.utils.ColoramaUtils import Style
...
class SenderEmailController:
    ...
    def __init__(self, data_user: LoginSchema, data_send: dict, colorTerminal: str) -> None:
    ...
    def read_emails(self) -> tuple[list[str], pd.DataFrame, list[str]]:
        if SenderEmailController.is_file_locked(self.data_send["spreadsheet_path"]):
            raise Exception(f"{self.colorTerminal}O arquivo de planilha está sendo utilizado por outro processo, feche-a e tente novamente.{Style.RESET_ALL}")
        ...
        try:
            if not {"E-mail", "STATUS", "PDF"}.issubset(self.list_columns):
                raise KeyError(f"{self.colorTerminal}Coluna 'E-mail' ou 'STATUS' ou 'PDF' não encontrada na planilha.{Style.RESET_ALL}")
            ...
            raise Exception(f"{self.colorTerminal}Erro ao carregar emails: {error}{Style.RESET_ALL}")
    ...
    def write_on_console_and_txt(
        ...
        if not validator:
            message: str = (
                f"{self.colorTerminal}Erro ao enviar email para {destination_email} às {datetime.now().strftime('%d/%m/%Y-%H:%M:%S')}{Style.RESET_ALL}\n" 
                f"\n{self.colorTerminal}Total de emails enviados: {current_index + 1} de {len(destination_emails)} Erro: {error}{Style.RESET_ALL}\n"
            )
            Log.write_error(self.data_user.email, message)
            return message

        message: str = (
            f"{self.colorTerminal}Email enviado para {destination_email} às {datetime.now().strftime('%d/%m/%Y-%H:%M:%S')}\nTotal de emails enviados: {current_index + 1} de {len(destination_emails)}"
            f"{self.colorTerminal}Intervalo de envio: {interval_send_email} segundos{Style.RESET_ALL}\n"
        )
    ...
    def error_env(
        ...
        print(f"{self.colorTerminal}{message_error}{Style.RESET_ALL}")
    ...
    def process_email_sending(
        ...
        try:
            ...
            self.error_env(df_emails, destination_email, destination_emails, current_index, message_error)
            return False
            ...
            self.error_env(df_emails, destination_email, destination_emails, current_index, error)
            return False
    ...
    def send_emails(self) -> None:
        ...
        if not destination_emails or start_index is None:
            raise Exception(f"{self.colorTerminal}Nenhum e-mail encontrado na planilha para envio. Cheque a coluna de E-mail ou STATUS.{Style.RESET_ALL}")
